...
uint8 Threshold_Comparator_initVar = 0u;

/* Internal functions definitoin */
static void Threshold_Comparator_trimAdjust(uint8 nibble) ;

/* static Threshold_Comparator_backupStruct  Threshold_Comparator_backup; */
#if (CY_PSOC5A)
    static Threshold_Comparator_LOWPOWER_BACKUP_STRUCT  Threshold_Comparator_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Threshold_Comparator_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Threshold_Comparator_Init
********************************************************************************
*
* Summary:
...
void Threshold_Comparator_Init(void) 
{
    ...
    Threshold_Comparator_SetSpeed(Threshold_Comparator_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Threshold_Comparator_DEFAULT_HYSTERESIS == 0u )
        Threshold_Comparator_CR |= Threshold_Comparator_HYST_OFF;
    #else
        Threshold_Comparator_CR &= (uint8)(~Threshold_Comparator_HYST_OFF);
    #endif /* Threshold_Comparator_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Threshold_Comparator_DEFAULT_PWRDWN_OVRD == 0u )
            Threshold_Comparator_CR &= (uint8)(~Threshold_Comparator_PWRDWN_OVRD);
        #else 
            Threshold_Comparator_CR |= Threshold_Comparator_PWRDWN_OVRD;
        #endif /* Threshold_Comparator_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Threshold_Comparator_CR |= Threshold_Comparator_MX_AO;

    /* Set default sync */
    Threshold_Comparator_CLK &= (uint8)(~Threshold_Comparator_SYNCCLK_MASK);
    #if ( Threshold_Comparator_DEFAULT_BYPASS_SYNC == 0u )
        Threshold_Comparator_CLK |= Threshold_Comparator_SYNC_CLK_EN;
    #else
        Threshold_Comparator_CLK |= Threshold_Comparator_BYPASS_SYNC;
    #endif /* Threshold_Comparator_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Threshold_Comparator_Enable
********************************************************************************
*
* Summary:
...
{
    Threshold_Comparator_PWRMGR |= Threshold_Comparator_ACT_PWR_EN;
    Threshold_Comparator_STBY_PWRMGR |= Threshold_Comparator_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Threshold_Comparator_restoreReg == 1u)
        {
            Threshold_Comparator_CR = Threshold_Comparator_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Threshold_Comparator_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: Threshold_Comparator_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Threshold_Comparator_initVar: Is modified when this function is called for the 
...
void Threshold_Comparator_Start(void) 
{
    ...
    if ( Threshold_Comparator_initVar == 0u )
    {
        Threshold_Comparator_Init();
        
        Threshold_Comparator_initVar = 1u;
    }   
...
/*******************************************************************************
* Function Name: Threshold_Comparator_Stop
********************************************************************************
*
* Summary:
...
void Threshold_Comparator_Stop(void) 
{
    ...
    Threshold_Comparator_PWRMGR &= (uint8)(~Threshold_Comparator_ACT_PWR_EN);
    Threshold_Comparator_STBY_PWRMGR &= (uint8)(~Threshold_Comparator_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Threshold_Comparator_restoreReg = 1u;

        /* Save the control register before clearing it */
        Threshold_Comparator_lowPowerBackup.compCRReg = Threshold_Comparator_CR;
        Threshold_Comparator_CR = Threshold_Comparator_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Threshold_Comparator_SetSpeed
********************************************************************************
*
* Summary:
...
void Threshold_Comparator_SetSpeed(uint8 speed) 
{
    ...
    Threshold_Comparator_CR = (Threshold_Comparator_CR & (uint8)(~Threshold_Comparator_PWR_MODE_MASK)) |
                           (speed & Threshold_Comparator_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Threshold_Comparator_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Threshold_Comparator_TR = Threshold_Comparator_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Threshold_Comparator_TR0 = Threshold_Comparator_HS_TRIM_TR0;
            Threshold_Comparator_TR1 = Threshold_Comparator_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Threshold_Comparator_TR = Threshold_Comparator_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Threshold_Comparator_TR0 = Threshold_Comparator_LS_TRIM_TR0;
            Threshold_Comparator_TR1 = Threshold_Comparator_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Threshold_Comparator_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Threshold_Comparator_WRK & Threshold_Comparator_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Threshold_Comparator_trimAdjust
********************************************************************************
*
* Summary:
...
static void Threshold_Comparator_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Threshold_Comparator_WRK & Threshold_Comparator_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Threshold_Comparator_TR |= Threshold_Comparator_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Threshold_Comparator_TR0 |= Threshold_Comparator_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Threshold_Comparator_TR |= Threshold_Comparator_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Threshold_Comparator_TR1 |= Threshold_Comparator_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        if (nibble == 0u)
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Threshold_Comparator_TR += 1u;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Threshold_Comparator_TR0 += 1u;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
        else
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Threshold_Comparator_TR += 0x10u;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Threshold_Comparator_TR1 += 1u;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
        ...
        if ((Threshold_Comparator_WRK & Threshold_Comparator_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: Threshold_Comparator_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 Threshold_Comparator_ZeroCal(void) 
{
    ...
    tmpSW0 = Threshold_Comparator_SW0;
    tmpSW2 = Threshold_Comparator_SW2;
    tmpSW3 = Threshold_Comparator_SW3;

     /* Clear routing for inP, retain routing for inN */
    Threshold_Comparator_SW0 = 0x00u;
    Threshold_Comparator_SW2 = 0x00u;
    Threshold_Comparator_SW3 = tmpSW3 & (uint8)(~Threshold_Comparator_CMP_SW3_INPCTL_MASK);

    /* Preserve original configuration
     * - turn off Hysteresis
     * - set calibration bit - shorts inN to inP
    */
    tmpCR = Threshold_Comparator_CR;
    Threshold_Comparator_CR |= (Threshold_Comparator_CAL_ON | Threshold_Comparator_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Threshold_Comparator_TR = Threshold_Comparator_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Threshold_Comparator_TR0 = Threshold_Comparator_DEFAULT_CMP_TRIM;
        Threshold_Comparator_TR1 = Threshold_Comparator_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Threshold_Comparator_CR & Threshold_Comparator_PWR_MODE_MASK) == Threshold_Comparator_PWR_MODE_FAST)
    {
        Threshold_Comparator_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Threshold_Comparator_trimAdjust(1u);
		Threshold_Comparator_trimAdjust(0u);
    }
   
    /* Restore Config Register */
    Threshold_Comparator_CR = tmpCR;
    
    /* Restore routing registers for inP */
    Threshold_Comparator_SW0 = tmpSW0;
    Threshold_Comparator_SW2 = tmpSW2;
    Threshold_Comparator_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Threshold_Comparator_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Threshold_Comparator_TR1 << 8u) | (Threshold_Comparator_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Threshold_Comparator_LoadTrim
********************************************************************************
*
* Summary:
...
void Threshold_Comparator_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Threshold_Comparator_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Threshold_Comparator_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
