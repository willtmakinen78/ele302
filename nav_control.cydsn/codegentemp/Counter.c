...
void Counter_Init(void) 
{
        ...
        /* Check if One Shot mode is enabled i.e. RunMode !=0*/
            ...
            Counter_CONTROL |= 0x04u;
        #endif /* One Shot enabled only when RunModeUsed is not Continuous*/

        #if (Counter_RunModeUsed == 2)
            #if (CY_PSOC5A)
                /* Set last 2 bits of control2 register if one shot(halt on
                interrupt) is enabled*/
                Counter_CONTROL2 |= 0x03u;
            #endif /* Set One-Shot Halt on Interrupt bit in CONTROL2 for PSoC5A */

            #if (CY_PSOC3 || CY_PSOC5LP)
                /* Set last 2 bits of control3 register if one shot(halt on
                interrupt) is enabled*/
                Counter_CONTROL3 |= 0x03u;
            #endif /* Set One-Shot Halt on Interrupt bit in CONTROL3 for PSoC3 or PSoC5LP */

        #endif /* Remove section if One Shot Halt on Interrupt is not enabled */

        #if (Counter_UsingHWEnable != 0)
            #if (CY_PSOC5A)
                /* Set the default Run Mode of the Timer to Continuous */
                Counter_CONTROL2 |= Counter_CTRL_MODE_PULSEWIDTH;
            #endif /* Set Continuous Run Mode in CONTROL2 for PSoC5A */

            #if (CY_PSOC3 || CY_PSOC5LP)
                /* Clear and Set ROD and COD bits of CFG2 register */
                Counter_CONTROL3 &= ((uint8)(~Counter_CTRL_RCOD_MASK));
                Counter_CONTROL3 |= Counter_CTRL_RCOD;

                /* Clear and Enable the HW enable bit in CFG2 register */
                Counter_CONTROL3 &= ((uint8)(~Counter_CTRL_ENBL_MASK));
                Counter_CONTROL3 |= Counter_CTRL_ENBL;

                /* Set the default Run Mode of the Timer to Continuous */
                Counter_CONTROL3 |= Counter_CTRL_MODE_CONTINUOUS;
    ...
    Counter_WritePeriod(Counter_INIT_PERIOD);
    Counter_WriteCounter(Counter_INIT_PERIOD);

    #if (Counter_UsingHWCaptureCounter)/* Capture counter is enabled */
        Counter_CAPTURE_COUNT_CTRL |= Counter_CNTR_ENABLE;
        Counter_SetCaptureCount(Counter_INIT_CAPTURE_COUNT);
    #endif /* Configure capture counter value */

    #if (!Counter_UsingFixedFunction)
        #if (Counter_SoftwareCaptureMode)
            Counter_SetCaptureMode(Counter_INIT_CAPTURE_MODE);
        #endif /* Set Capture Mode for UDB implementation if capture mode is software controlled */

        #if (Counter_SoftwareTriggerMode)
            #if (!Counter_UDB_CONTROL_REG_REMOVED)
                if (0u == (Counter_CONTROL & Counter__B_TIMER__TM_SOFTWARE))
                {
                    Counter_SetTriggerMode(Counter_INIT_TRIGGER_MODE);
                }
        ...
        CyExitCriticalSection(Counter_interruptState);
            ...
            Counter_EnableTrigger();
        #endif /* Set Trigger enable bit for UDB implementation in the control register*/
		
		
        #if (Counter_InterruptOnCaptureCount && !Counter_UDB_CONTROL_REG_REMOVED)
            Counter_SetInterruptCount(Counter_INIT_INT_CAPTURE_COUNT);
        #endif /* Set interrupt count in UDB implementation if interrupt count feature is checked.*/

        Counter_ClearFIFO();
    #endif /* Configure additional features of UDB implementation */

    Counter_SetInterruptMode(Counter_INIT_INTERRUPT_MODE);
}
...
void Counter_Enable(void) 
{
    ...
    #if (!Counter_UDB_CONTROL_REG_REMOVED || Counter_UsingFixedFunction)
        Counter_CONTROL |= Counter_CTRL_ENABLE;
...
{
    Counter_STATUS_MASK = interruptMode;
}


/*******************************************************************************
* Function Name: Counter_SoftwareCapture
********************************************************************************
*
* Summary:
...
#if (!Counter_UDB_CONTROL_REG_REMOVED) /* Remove API if control register is unused */
...
void Counter_WritePeriod(uint16 period) 
{
        ...
        CY_SET_REG16(Counter_PERIOD_LSB_PTR, period_temp);
    #else
...
{
   #if(Counter_UsingFixedFunction)
       return ((uint16)CY_GET_REG16(Counter_CAPTURE_LSB_PTR));
   #else
       return (CY_GET_REG16(Counter_CAPTURE_LSB_PTR));
   #endif /* (Counter_UsingFixedFunction) */
}
...
#if(!Counter_UsingFixedFunction) /* UDB Specific Functions */
...
*  Counter__B_TIMER__CM_NONE        - Set Capture mode to None
*  Counter__B_TIMER__CM_RISINGEDGE  - Rising edge of Capture input
*  Counter__B_TIMER__CM_FALLINGEDGE - Falling edge of Capture input
*  Counter__B_TIMER__CM_EITHEREDGE  - Either edge of Capture input
*
* Return:
*  void
*
...
void Counter_SetCaptureMode(uint8 captureMode) 
{
    ...
    captureMode = ((uint8)((uint8)captureMode << Counter_CTRL_CAP_MODE_SHIFT));
    captureMode &= (Counter_CTRL_CAP_MODE_MASK);

    #if (!Counter_UDB_CONTROL_REG_REMOVED)
        /* Clear the Current Setting */
        Counter_CONTROL &= ((uint8)(~Counter_CTRL_CAP_MODE_MASK));

        /* Write The New Setting */
        Counter_CONTROL |= captureMode;
    #endif /* (!Counter_UDB_CONTROL_REG_REMOVED) */
}
...
/*******************************************************************************
* Function Name: Counter_SetTriggerMode
********************************************************************************
*
* Summary:
...
{
    /* This must only set to two bits of the control register associated */
    triggerMode &= Counter_CTRL_TRIG_MODE_MASK;

    #if (!Counter_UDB_CONTROL_REG_REMOVED)   /* Remove assignment if control register is removed */
    
        /* Clear the Current Setting */
        Counter_CONTROL &= ((uint8)(~Counter_CTRL_TRIG_MODE_MASK));

        /* Write The New Setting */
        Counter_CONTROL |= (triggerMode | Counter__B_TIMER__TM_SOFTWARE);
...
/*******************************************************************************
* Function Name: Counter_EnableTrigger
********************************************************************************
*
* Summary:
...
{
    #if (!Counter_UDB_CONTROL_REG_REMOVED)   /* Remove assignment if control register is removed */
        Counter_CONTROL |= Counter_CTRL_TRIG_EN;
    #endif /* Remove code section if control register is not used */
}


/*******************************************************************************
* Function Name: Counter_DisableTrigger
********************************************************************************
*
* Summary:
...
{
    #if (!Counter_UDB_CONTROL_REG_REMOVED )   /* Remove assignment if control register is removed */
        Counter_CONTROL &= ((uint8)(~Counter_CTRL_TRIG_EN));
    #endif /* Remove code section if control register is not used */
}
...
/*******************************************************************************
* Function Name: Counter_SetInterruptCount
********************************************************************************
*
* Summary:
...
{
    /* This must only set to two bits of the control register associated */
    interruptCount &= Counter_CTRL_INTCNT_MASK;

    #if (!Counter_UDB_CONTROL_REG_REMOVED)
        /* Clear the Current Setting */
        Counter_CONTROL &= ((uint8)(~Counter_CTRL_INTCNT_MASK));
        /* Write The New Setting */
        Counter_CONTROL |= interruptCount;
    #endif /* (!Counter_UDB_CONTROL_REG_REMOVED) */
}
...
/*******************************************************************************
* Function Name: Counter_SetCaptureCount
********************************************************************************
*
* Summary:
...
{
    Counter_CAP_COUNT = captureCount;
}


/*******************************************************************************
* Function Name: Counter_ReadCaptureCount
********************************************************************************
*
* Summary:
...
{
    return ((uint8)Counter_CAP_COUNT);
}
